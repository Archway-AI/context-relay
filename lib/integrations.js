...
function parseAgentConfig(existing) {
  ...
  if (Object.hasOwn(value, "hooks") && !isPlainJsonObject(value.hooks)) {
    return { kind: "invalid", error: "hooks must contain a JSON object" };
  }
  if (value.hooks && Object.hasOwn(value.hooks, "PreToolUse") && !Array.isArray(value.hooks.PreToolUse)) {
    return { kind: "invalid", error: "hooks.PreToolUse must contain a JSON array" };
  }
