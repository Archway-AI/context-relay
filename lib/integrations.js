...
// Recognizes both the legacy bare-name hook command ("context-relay hook claude") and the
// current absolute-path form ("<node> <script> hook claude") so that an install written by
// an older version still detects as installed (no duplicate on re-init) and still uninstalls
// cleanly. Deliberately conservative: requires the command to END with "hook <provider>" as
// its own word (so "unhook claude" or "hook claude-thing" don't match) AND to reference
// "context-relay" somewhere in the command, so an unrelated tool's same-shaped hook is not
// swept up.
export function isManagedHookCommand(command, provider) {
  ...
  if (!new RegExp(`(?:^|\\s)hook\\s+${provider}$`).test(trimmed)) {
    return false;
  }
  return /context-relay/i.test(trimmed);
}
...
function isAllowedCommandShape(command, executable) {
  ...
  if (executable === "npm" || executable === "pnpm") {
    const subcommandIndex = findNpmSubcommandIndex(parts);
    if (subcommandIndex === -1) {
      return false;
    }
...
function mergeClaudeSettings(existing, hookCommand) {
  ...
  const hasHook = preToolUse.some((entry) => {
    ...
    return entry.hooks.some((hook) => hook?.type === "command" && isManagedHookCommand(hook?.command, "claude"));
  });
...
function removeClaudeHook(existing) {
  ...
  const nextPreToolUse = preToolUse
    .map((entry) => {
      ...
      return {
        ...entry,
        hooks: entry.hooks.filter((hook) => hook?.type !== "command" || !isManagedHookCommand(hook?.command, "claude")),
      };
    })
...
function mergeCodexHooks(existing, hookCommand) {
  ...
  const hasHook = preToolUse.some((entry) => {
    ...
    return entry.hooks.some((hook) => hook?.type === "command" && isManagedHookCommand(hook?.command, "codex"));
  });
  const nextPreToolUse = hasHook
    ? preToolUse
    : [
...
function removeCodexHook(existing) {
  ...
  const nextPreToolUse = preToolUse
    .map((entry) => {
      ...
      return {
        ...entry,
        hooks: entry.hooks.filter((hook) => hook?.type !== "command" || !isManagedHookCommand(hook?.command, "codex")),
      };
    })
    .filter((entry) => !Array.isArray(entry?.hooks) || entry.hooks.length > 0);
  const nextHooks = { ...hooks };
