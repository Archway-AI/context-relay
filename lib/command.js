...
function capturedDurationMs(startedAt, env) {
  const fixed = env.CONTEXT_RELAY_FIXED_DURATION_MS;
  if (fixed !== undefined) {
    const duration = Number(fixed);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code, signal) => {
      ...
      resolve({
        ...
        signal,
        rawText,
        durationMs: capturedDurationMs(startedAt, env),
      });
    });
  });
}
