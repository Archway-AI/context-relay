...
// Runs-relative rates are reported as ratios rounded to three decimals.
// Zero runs reports 0 so callers never see NaN or Infinity.
function ratePerRun(count, runs) {
  return runs > 0 ? Number((count / runs).toFixed(3)) : 0;
}
...
export class ArtifactStore {
  ...
  async get(artifactId) {
    let text;
    try {
      text = await readFile(this.artifactPath(artifactId), "utf8");
    } catch (error) {
      if (!isMissing(error)) {
        throw error;
      }
    ...
    try {
      ...
      const corrupt = new Error(`CR_RETRIEVE_CORRUPT_JSON: artifact ${artifactId} contains invalid JSON`, {
        cause: error,
      });
      corrupt.code = "CR_RETRIEVE_CORRUPT_JSON";
      throw corrupt;
    }
  ...
  async readStats() {
    ...
    for (const event of events) {
      ...
      if (event.reasonCode === "CR_STORE_FAILED") {
        stats.fallbacks += 1;
      }
    ...
    // compression_savings_pct is an alias for gross_efficiency_percent, not a
    // second computation. See docs/evals.md for the metric definitions.
    stats.compression_savings_pct = stats.gross_efficiency_percent;
    stats.retrieval_rate = ratePerRun(stats.retrievals, stats.runs);
    stats.blocked_rate = ratePerRun(stats.blocked, stats.runs);
    stats.fallback_rate = ratePerRun(stats.fallbacks, stats.runs);
    return stats;
  }
