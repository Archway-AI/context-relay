...
function discoverPayload(stats, events, agentStatus) {
  ...
  for (const [provider, flag] of [["Claude Code", "claude"], ["Codex", "codex"]]) {
    ...
    if (agent.configState === "invalid") {
      setup.push(`${provider} configuration is invalid. Repair it before running \`context-relay init --${flag}\`.`);
      continue;
    }
...
export async function main(args) {
  ...
  if (command === "run") {
    ...
    const optionEnd = separator === -1 ? rawArgs.length : separator;
    await runCommand([...rawArgs.slice(0, optionEnd), "--mode", "raw", ...rawArgs.slice(optionEnd)], store);
