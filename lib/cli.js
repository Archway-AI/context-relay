...
import { classifyCommand, hasSecret, redactCommandArg, redactSecrets } from "./policy.js";
...
async function runCommand(args, store) {
  ...
  if (policy.mode === "blocked") {
    ...
    const redactedText = redactSecrets(result.rawText);
    ...
    if (blockedArtifact) {
      reasonCode = "CR_BLOCK_SECRET";
      summary = `Secret-shaped content was detected and replaced with [REDACTED_SECRET] before storage. Retrieval returns the redacted text, never the original secret.\n${summarize({
